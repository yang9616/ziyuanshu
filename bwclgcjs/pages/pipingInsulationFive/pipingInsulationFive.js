...
Page({
    ...
    startInput(e) {
        ...
        let terminalT = this.data.form.terminalT;
        let heatcpt = this.data.form.heatcpt;
        if (et && ply && windSpeed && pipelineOuter && flow && length && heatcpt) {
            let windSpeed2 = parseFloat(windSpeed);
            let meanTemp = parseFloat(((ply + terminalT) / 2).toFixed(2));
            let hotcft = parseFloat((11.63 + 7 * (Math.pow(windSpeed2, 0.5))).toFixed(3));
            let x = ((ply - et) / (terminalT - et)).toFixed(2);
            //导热系数
    ...
    submit(e) {
        ...
        let x = this.data.form.x;
        let hotcft = this.data.form.hotcft;
        if (nowText && hotcft && x && crtFactor) {
            if (x >= 2) {
                let xx = (2 * 3.1416 * nowText * (3.6 * (terminalT - et) * length * crtFactor / (flow * heatcpt * (ply - et) - 1000 / (3.1416 * pipelineOuter * hotcft)))).toFixed(9);
                let hpPly = (pipelineOuter * (Math.exp(xx) - 1) / 2).toFixed(0);
                this.setData({
                    form: {
                        ...this.data.form,
                        hpPly
                ...
                let xx = (2 * 3.1416 * nowText * (3.6 * (meanTemp - et) * length * crtFactor / (flow * heatcpt * (ply - terminalT)) - 1000 / (3.1416 * pipelineOuter * hotcft))).toFixed(9);
                let hpPly = ((pipelineOuter * (Math.exp(xx) - 1) / 2)).toFixed(0);
                this.setData({
                    form: {
                        ...this.data.form,
