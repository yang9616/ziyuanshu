...
Page({
    ...
    startInput(e) {
        ...
        let pst = this.data.form.pst;
        let pipelineOuter = this.data.form.pipelineOuter;
        let depth = this.data.form.depth;
        let stc = this.data.form.stc;
        if (et && ply && pst && depth && pipelineOuter && stc) {
            let meanTemp = ((ply + pst) / 2).toFixed(2);
            //导热系数
    ...
    submit(e) {
        ...
        if (nowText && meanTemp) {
            let lnd1 = (stc * (pst - et) * Math.log(pipelineOuter / 1000) + nowText * (ply - pst) * Math.log(4 * depth)) / (nowText * (ply - pst) + stc * (pst - et));
            let int = ((1000 * Math.exp(lnd1) - pipelineOuter) / 2).toFixed(0);
            this.setData({
                error: false,
                form: {
                    ...this.data.form,
