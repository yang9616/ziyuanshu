...
Page({
    ...
    submit(e) {
        ...
        let pipelineOuter = this.data.form.pipelineOuter; //35
        let hotcft = parseFloat(this.data.form.hotcft); //37
        let nowText = parseFloat(this.data.nowText); //38
        if (et && ply && hpPly && pipelineOuter && nowText) {
            let m = (3.1416 * (ply - et) / (Math.log((2 * hpPly + pipelineOuter) / pipelineOuter) / (2 * nowText) + 1 / (hotcft * (2 * hpPly + pipelineOuter) / 1000))).toFixed(2);
            let square = (m / 3.1416 / (2 * hpPly + pipelineOuter) * 1000).toFixed(2);
