...
Page({
    ...
    data: {
        ...
        itemTxtStart: [
            ["环境温度（（摄氏度(℃））", "et"], //18
            ["介质温度 (（摄氏度(℃）)", "ply"], //19
            ["风速 (米（m/s）)", "windSpeed"], //20
            ["管道外径 (mm)", "pipelineOuter"], //21
            ["保温后外表温度 (（摄氏度(℃）)", "etai"], //22

        ],
        itemTxt: [
            ["平均温度 (摄氏度(℃）)", "meanTemp"], //23
            ["于大气的换热系数 (W/(m².K))", "hotcft"], //24
        ],
        result: ["保温层厚度 （mm）"],
        select: [
    ...
    startInput(e) {
        ...
        let etai = this.data.form.etai;
        if (et && ply && windSpeed && pipelineOuter && etai) {
            let windSpeed2 = parseFloat(windSpeed);
            let meanTemp = (0.514 * (ply + etai)).toFixed(2);
    ...
    submit(e) {
        ...
        if (nowText && hotcft) {
            let lnx = ((2 * nowText) / (hotcft * pipelineOuter / 1000) * ((ply - etai) / (etai - et))).toFixed(4);
            this.getX(lnx);
