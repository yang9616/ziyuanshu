...
Page({
    ...
    startInput(e) {
        ...
        let ply2 = (this.data.form.ply).toString();
        let dosage = (this.data.form.dosage).toString();
        let mTemp = this.data.form.mTemp;
        if (diameter && ply2 && dosage && mTemp) {
            let ply = parseFloat(ply2)
            console.log('ply', typeof ply)
            let dosage2 = parseFloat(dosage);
            let meanTemp = (0.514 * (ply + mTemp)).toFixed(2);
            let hotcft = (11.63 + 7 * (Math.pow(dosage2, 0.5))).toFixed(2);
            //导热系数
    ...
    submit(e) {
        ...
        if (diameter && dosage && mTemp && hotcft && nowText) {
            let tempPly = ((1000 * nowText / hotcft * ((ply - mTemp) / (mTemp - diameter)))).toFixed(0);
            this.setData({
                error: false,
                form: {
                    ...this.data.form,
